...
def get_k_closest_documents(dictionary, normalized_idfs, data, k, approx_k):
    normalized_idfs_noise_reduced = use_svd_and_low_rank_approx(
        normalized_idfs, approx_k)
    ...
    for document in normalized_idfs_noise_reduced:
        values = document
        cos = (q_vector_T @ np.array(values)) / \
            (np.linalg.norm(q_vector) * np.linalg.norm(values))
...
f.close()
documents = get_k_closest_documents(
    dictionary, normalized_idfs, query, k, approx_k)
print(documents)
sys.stdout.flush()
